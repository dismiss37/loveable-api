...
def majorlogin_jwt():
    ...
    # ✅ MULTI-THREADED PLATFORM LOOP (20 THREADS) - यही SPEED का RAZ है
    def try_platform(platform_type):
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = {executor.submit(try_platform, pt): pt for pt in platforms}
...
# ✅ Bulk POST endpoint (JSON array) – for future use
@app.route('/bulk-token', methods=['POST'])
def bulk_token():
    data = request.get_json()
    if not data or not isinstance(data, list):
        return jsonify({"error": "Expected JSON array of {uid, password}"}), 400
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        future_to_account = {
            executor.submit(process_single_oauth, item['uid'], item['password']): item
            for item in data
        }
...
def process_single_oauth(uid, password):
    oauth_url = "https://100067.connect.garena.com/oauth/guest/token/grant"
    payload = {
        'uid': uid,
        'password': password,
    ...
    if oauth_response.status_code != 200:
        try:
            ...
            error_data = {"message": oauth_response.text}
        return {"uid": uid, "error": error_data}

    try:
        oauth_data = oauth_response.json()
    except ValueError:
        return {"uid": uid, "error": "Invalid JSON response from OAuth service"}

    if 'access_token' not in oauth_data or 'open_id' not in oauth_data:
        return {"uid": uid, "error": "OAuth response missing access_token or open_id"}

    with app.test_request_context('/access-jwt', query_string={
        'access_token': oauth_data['access_token'],
        'open_id': oauth_data['open_id']
    }):
        response, status_code = majorlogin_jwt()
        if status_code == 200:
            try:
                return response.get_json()
            except:
                return {"uid": uid, "error": "Failed to parse response"}
        else:
            try:
                error_msg = response.get_json()
            except:
                error_msg = {"message": "Unknown error"}
            return {"uid": uid, "error": error_msg}

# ✅ IMPORTANT: यह लाइन आपके पसंदीदा URL को SUPPORT करती है
# अब आप http://127.0.0.1:7860/bulk-token?uid=...&password=... भी USE कर सकते हैं
app.add_url_rule('/bulk-token', view_func=oauth_guest, methods=['GET'])
